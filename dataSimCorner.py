...
# Tf = 30e-6 # 30 yrs
# m22_ = 2.5 # run1
m22_ = 10.0000001 # run2
print(m22_, simName)
m22 = np_.array([m22_])
if gpu:
	m22 = np.asarray(m22) 
L = 2.2#5. / m22_ *N/256 # kpc box
dx = L / N
C = au.G * 4. * np.pi
cf = 0.1
...
lam_db = 2*np.pi* hbar_ / (sigma_dm)
tau_db = lam_db / sigma_dm
Tf = 1.*tau_db
print(lam_db/dx)
print(tau_db * 1e6)
print(2*np.pi* hbar_ / (au.speed_of_light)**2)
...
def SetPulsarICs(s):
	r_earth = np.array([-L/2.1,-L/2.1,-L/2.1])
	...
	points = np.abs(RandomOnSphere(N_pulsars, 1., 0.01))
	...
	for i in range(N_pulsars):
		...
		pulsar_.r = np.array([xPos, yPos, zPos]) + r_earth
